...
const { runCandidatureAndIdentification } = require('./openEudonetNewCandidature');
...
function parseArgs(argv) {
  const options = {
    ...
    fullProcess: String(process.env.EUDONET_FULL_PROCESS || '') === '1',
  };
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    } else if (arg === '--full-process') {
      options.fullProcess = true;
...
function autoLetterSuffix(date = new Date()) {
  const letters = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ';
  const index = Math.floor(date.getTime() / 60000) % letters.length;
  return letters[index];
}

function autoContactIdentity(source, date = new Date()) {
  const listIndex = Math.floor(date.getTime() / 60000) % realisticAutoContacts.length;
  const [fallbackLastName, fallbackFirstName] = realisticAutoContacts[listIndex];
  const marker = String(source.marqueAuto || autoLetterSuffix(date)).trim().slice(0, 1).toUpperCase();
  const useProvidedName = source.figerNomAuto === true;
  const lastNameBase = String(useProvidedName ? source.nomContact : fallbackLastName).trim();
  const firstNameBase = String(useProvidedName ? source.prenom : fallbackFirstName).trim();
  return {
    nomContact: `${lastNameBase}${marker}`,
    prenom: `${firstNameBase}${marker}`,
  };
}
...
function loadContact(dataPath) {
  ...
  const generatedIdentity = uniqueData
    ? autoContactIdentity(source)
    : { nomContact: source.nomContact, prenom: source.prenom };
  const contact = {
    nomContact: titleCaseName(generatedIdentity.nomContact),
    prenom: titleCaseName(generatedIdentity.prenom),
  ...
  if (!contact.courriel) {
    const emailSuffix = String(Date.now()).slice(-4);
    const emailDomain = String(source.domaineCourrielAuto || process.env.EUDONET_EMAIL_DOMAIN || 'test.com')
      .trim()
      .replace(/^@+/, '');
    contact.courriel = `zzerpiris${emailPart(contact.nomContact)}${emailPart(contact.prenom)}${emailSuffix}@${emailDomain}`;
  }
...
async function hoverContacts(context) {
  ...
  while (Date.now() < deadline) {
    contacts = await firstVisibleLocator(context, [
      ...
      (frame) => frame.locator('a, button, li, div, span').filter({ hasText: /^Contacts$/i }),
    ...
    for (const frame of allFrames(context)) {
      const hovered = await frame.evaluate(() => {
        ...
        const target = Array.from(document.querySelectorAll('a, button, li, div, span'))
          .filter((element) => visible(element) && normalize(element.textContent) === 'contacts')
          .sort((left, right) => {
            const leftRect = left.getBoundingClientRect();
            const rightRect = right.getBoundingClientRect();
            return (leftRect.width - rightRect.width) || (leftRect.left - rightRect.left);
          })[0];
        if (!target) return false;
      ...
      if (hovered) {
        log(`Menu Contacts survole via DOM dans la frame: ${frame.url() || 'main'}`);
...
async function fillContactSearch(context, value) {
  ...
  while (!searchInput && Date.now() < deadline) {
    searchInput = await firstVisibleLocator(context, [
    ...
    if (!searchInput) await new Promise((resolve) => setTimeout(resolve, 500));
  }
  ...
  if (!searchInput) {
    ...
    await activePage.keyboard.type(value, { delay: 20 }).catch(() => null);
    log(`Recherche Contacts saisie via fallback visuel: ${value}`);
    return;
  }
...
async function isContactCreationFormOpen(context) {
  const technicalField = await firstVisibleLocator(context, [
    (frame) => frame.locator('#COL_200_201_0_0_0'),
    (frame) => frame.locator('input[name="COL_200_201_0_0_0"]'),
  ]);
  if (technicalField) return true;
  ...
  for (const frame of allFrames(context)) {
    const found = await frame.evaluate(() => {
      ...
      if (!pageText.includes('nouvelle fiche contacts')) return false;
      return Array.from(document.querySelectorAll('label, span, div, td'))
        .some((element) => visible(element) && normalize(element.textContent).includes('nom contact'));
    }).catch(() => false);
...
async function clickAddButton(context) {
  await activePage.mouse.click(230, 303).catch(() => null);
  await activePage.waitForTimeout(1200).catch(() => null);
  if (await isContactCreationFormOpen(context)) {
    log('Clic positionnel effectue sur le bouton + Ajouter de la popup Contacts.');
    return;
  }

  const loaderDeadline = Date.now() + 30000;
  ...
  for (const frame of allFrames(context)) {
    const clicked = await frame.evaluate(() => {
      const target = document.querySelector('.lnkFieldsAdd[onclick*="AddFileFromPopup"]');
      if (!target) return false;
      target.dispatchEvent(new MouseEvent('mousedown', { bubbles: true, cancelable: true, view: window }));
      target.dispatchEvent(new MouseEvent('mouseup', { bubbles: true, cancelable: true, view: window }));
      target.click();
    ...
    if (clicked) {
      ...
      if (await isContactCreationFormOpen(context)) {
        log(`Clic DOM direct effectue sur lnkFieldsAdd dans la frame: ${frame.url() || 'main'}`);
        return;
      }
    }
  }
  ...
  for (const frame of allFrames(context)) {
    const clicked = await frame.evaluate(() => {
      ...
      const popup = Array.from(document.querySelectorAll('*'))
        .find((element) => visible(element) && normalize(element.textContent).startsWith('ajouter rechercher'));
      const scope = popup || document;
      const candidates = Array.from(scope.querySelectorAll('li, a, button, div, span'))
        ...
        .filter((item) => (
          item.text === 'ajouter'
          || item.title === 'ajouter'
          || /(^|\s)(icon-add|fa-plus|icnBkm)(\s|$)/.test(item.cls)
        ))
        .filter((item) => item.rect.top > 250 && item.rect.top < 370)
        .sort((left, right) => left.rect.left - right.rect.left);
      const target = candidates[0]?.element;
      if (!target) return false;
      const clickable = target.closest('li, a, button, div[onclick], span[onclick]') || target;
    ...
    if (clicked) {
      await activePage.waitForTimeout(1500).catch(() => null);
      if (await isContactCreationFormOpen(context)) {
        log(`Clic DOM effectue sur le bouton Ajouter precis dans la frame: ${frame.url() || 'main'}`);
        return;
      }
    }
  }
  try {
    ...
    if (await isContactCreationFormOpen(context)) {
      log(`Bouton Ajouter a ouvert la fiche malgre un clic intercepte: ${error.message || error}`);
      return;
    }
    throw error;
  }
...
async function main() {
  ...
  if (options.submit) {
    await activePage.goto(eudonetUrl, { waitUntil: 'domcontentloaded', timeout: 60000 });
    await activePage.waitForLoadState('networkidle', { timeout: 30000 }).catch(() => null);
    log(`Page rechargee apres verification initiale: ${activePage.url()}`);
  }
  ...
  if (options.submit) {
    ...
    if (options.fullProcess) {
      activePage = resultPage;
      log('Parcours complet active: enchainement Candidature puis Identification sans fermer Eudonet.');
      await runCandidatureAndIdentification(context, activePage, contact, {
        submit: true,
        withIdentification: true,
        identificationSubmit: true,
      });
      log('Parcours complet Eudonet termine.');
    }
  } else {
